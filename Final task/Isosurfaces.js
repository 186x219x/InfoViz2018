...
$(function() {
  ...
  $('#lambertianreflection').on('click', function() {
    var ref_color = new THREE.Color().setHex(cmap[isovalue][1]);
    var material_test = new THREE.ShaderMaterial({
      ...
      uniforms:{
	    ...
	    m_color: { type : 'v3', value: ref_color}
	}
    });
    material_test.color = color;
    return new THREE.Mesh( geometry, material_test );
  });
});
...
function IsosurfacesWithShading(volume, isovalue){
  var geometry = new THREE.Geometry();
  ...
  var material_Gouraud_Blinn_Phong = new THREE.ShaderMaterial({
    ...
    uniforms:{
    //light_position: { type : 'v3', value: screen.light.position },
    m_color: { type : 'v3', value: color}
}
  });


  material_Gouraud_Blinn_Phong.color = color;
  return new THREE.Mesh( geometry, material_Gouraud_Blinn_Phong );
