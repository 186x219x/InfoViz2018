function main()
{
    ...
    {
      var id = faces[i];
      var S0 = changeScalar(scalars[ id[0] ]);
      var S1 = changeScalar(scalars[ id[1] ]);
      var S2 = changeScalar(scalars[ id[2] ]);
    ...
    /*
    0.1~0.8の数を引数として指定すると、0~255の対応する数を返す関数
    */
    function changeScalar(scalar){
      var min = 0.1;
      var max = 0.8;
      var d = (max-min)/255;
      ...
      var changedScalar = 0;
      for(var i = 0; i<255; i++){
        array[i] = Math.abs(min - scalar);
        min = min + d;
      }
      array[255] = Math.abs(min - scalar);
